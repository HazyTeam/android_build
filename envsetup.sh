...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Hazy github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var HAZY_VERSION)
    ...
    if (adb shell getprop ro.hazy.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.hazy.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
